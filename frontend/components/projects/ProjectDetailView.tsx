          ...
          <div className="sticky top-4 border rounded-xl bg-card overflow-hidden">
            ...
            <div className="px-5 pt-4 pb-4 border-b border-border/40">
              <div className="flex items-start gap-3 mb-3">
                <div className="h-9 w-9 rounded-full shrink-0 flex items-center justify-center text-white font-bold text-[13px]" style={{ backgroundColor: accentColor }}>
                  {projectInitials}
                </div>
                <div className="min-w-0 flex-1 pt-0.5">
                  <h2 className="text-sm font-semibold leading-tight line-clamp-2 text-foreground">{project.name}</h2>
                  {project.client && (
                    <Link href={`/clients/${project.clientId}`} className="text-[11px] text-muted-foreground hover:text-primary truncate mt-0.5 block">
                      {project.client.name}
                    </Link>
                  )}
                  {project.jobNumber && (
                    <p className="text-[10px] text-muted-foreground/60 mt-0.5 font-mono">{project.jobNumber}</p>
                  )}
                </div>
              </div>
              <div className="flex flex-wrap gap-1.5">
                <span className={`inline-flex items-center text-xs font-semibold rounded-full px-2 py-0.5 border ${STATUS_CHIP[project.status] ?? 'text-gray-600 bg-gray-100 border-gray-200'}`}>
                  {project.status}
                </span>
                <span className={`inline-flex items-center gap-1 text-xs font-semibold rounded-full px-2 py-0.5 border ${risk.classes}`}>
            ...
            <div className="px-5 py-4 border-b border-border/40">
              ...
              <div className="grid grid-cols-2 gap-px bg-border/40 rounded-lg overflow-hidden mb-3">
                <div className="bg-card px-3 py-2.5 space-y-0.5">
                  <p className="text-[9px] uppercase tracking-[0.06em] text-muted-foreground font-medium">Contracted</p>
                  <p className="text-[18px] font-bold tabular-nums leading-none text-foreground">${fmtVal(contractedValue)}</p>
                </div>
                <div className="bg-card px-3 py-2.5 space-y-0.5">
                  <p className="text-[9px] uppercase tracking-[0.06em] text-muted-foreground font-medium">EOP Value</p>
                  {rawEndOfProjectValue != null ? (
                    <p className="text-[18px] font-bold tabular-nums leading-none text-blue-600">${fmtVal(rawEndOfProjectValue)}</p>
                  ) : (
                    <p className="text-[18px] font-bold leading-none text-muted-foreground/40">—</p>
                  )}
                </div>
              </div>
              <div className="flex gap-1.5 mb-3">
                {variance != null && variancePercent != null && (
                  <span className={`inline-flex items-center gap-1 text-[10px] font-semibold rounded-full px-2 py-0.5 border ${variance >= 0 ? 'text-emerald-700 bg-emerald-50 border-emerald-200' : 'text-red-700 bg-red-50 border-red-200'}`}>
                    {variance >= 0 ? <TrendingUp className="h-2.5 w-2.5" /> : <TrendingDown className="h-2.5 w-2.5" />}
                    {variance >= 0 ? '+' : ''}${fmtVal(Math.abs(variance))} ({variancePercent.toFixed(1)}%)
                  </span>
                )}
                <span className={`inline-flex items-center text-[10px] font-semibold rounded-full px-2 py-0.5 border ${margin >= 0 ? 'text-emerald-700 bg-emerald-50 border-emerald-200' : 'text-red-700 bg-red-50 border-red-200'}`}>
                  {margin.toFixed(1)}% margin
                </span>
              </div>
              {cost > 0 && <StatRow label="Total Cost">${fmtVal(cost)}</StatRow>}
            ...
            <div className="px-5 py-4 border-b border-border/40">
              ...
              <div className="space-y-2">
                ...
                ] as { role: string; person?: { name: string } | null }[]).map(({ role, person }) => (
                  <div key={role} className="flex items-center gap-2">
                    {person ? (
                      <>
                        <div className="h-5 w-5 rounded-full bg-secondary flex items-center justify-center shrink-0 text-secondary-foreground font-bold" style={{ fontSize: '7px' }}>
                          {avatarInitials(person.name)}
                        </div>
                        <span className="text-[11px] text-foreground flex-1 truncate">{person.name}</span>
                        <span className="text-[10px] text-muted-foreground shrink-0">{role}</span>
                      </>
                    ) : (
                      <>
                        <div className="h-5 w-5 rounded-full bg-muted shrink-0" />
                        <span className="text-[11px] text-muted-foreground flex-1">Unassigned</span>
                        <span className="text-[10px] text-muted-foreground shrink-0">{role}</span>
                      </>
                    )}
                  </div>
            ...
            {/* Timeline */}
            {(project.startDate || project.estimatedDueDate || project.completedDate) && (
              <div className="px-5 py-4 border-b border-border/40">
            ...
            <div className="px-5 py-4">
              ...
              <div className="grid grid-cols-2 gap-px bg-border/40 rounded-lg overflow-hidden">
                {[
                  { label: 'In Status', value: `${daysInStatus}d`, icon: Clock },
                  { label: 'Activities', value: activities.length, icon: Activity },
                  { label: 'Files', value: files.length, icon: FileText },
                  { label: 'Cost Logs', value: costLogs.length, icon: DollarSign },
                ].map((m) => {
                  const Icon = m.icon;
                  return (
                    <div key={m.label} className="bg-card px-2.5 py-2.5 text-center">
                      <p className="text-[18px] font-bold tabular-nums leading-none text-foreground mb-1">{m.value}</p>
            ...
            {hasPermission('projects:edit') && (
              <div className="px-5 py-4 border-t border-border/40">
                <Button variant="outline" size="sm" onClick={() => setIsEditOpen(true)} className="w-full justify-start gap-2 text-xs">
