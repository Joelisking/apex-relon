...
export function CustomerActivityTimeline({
  ...
  const handleAddActivity = async () => {
    ...
    try {
      ...
      await clientActivitiesApi.createActivity(clientId, data);
    ...
    <div className="space-y-3">
      ...
      {activities.length === 0 ? (
        ...
        <div className="relative space-y-2">
          <div className="absolute left-[9px] top-5 bottom-5 w-px bg-border/40 pointer-events-none" />
          ...
          {activities.map((activity) => {
            ...
            return (
              <div key={activity.id} className="flex gap-3">
                <div
                  className={`mt-[14px] h-[10px] w-[10px] rounded-full shrink-0 ring-2 ring-background ${nodeColor}`}
                />
      ...
      <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Record an activity for this client.
            </DialogDescription>
          </DialogHeader>
