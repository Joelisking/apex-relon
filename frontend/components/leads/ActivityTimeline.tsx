...
export function ActivityTimeline({
  ...
  // Set default activity type to first option when loaded
  useEffect(() => {
    if (
  ...
  // Set default meeting type to first option when loaded
  useEffect(() => {
    if (
  ...
  const handleAddActivity = async () => {
    ...
    try {
      ...
      if (hasMeetingSubtype)
        data.meetingType = meetingType as 'in-person' | 'virtual';

      await activitiesApi.createActivity(leadId, data);
          ...
          {activities.map((activity) => {
            ...
            return (
              <div key={activity.id} className="flex gap-3">
                <div
                  className={`mt-3.5 h-[10px] w-[10px] rounded-full shrink-0 ring-2 ring-background ${nodeColor}`}
                />
      ...
      <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Record an activity for this prospective project.
            </DialogDescription>
          </DialogHeader>
