...
export function LeadSwitcher({ currentLeadId, currentLeadName }: LeadSwitcherProps) {
          ...
          <div className="max-h-72 overflow-y-auto py-1">
            {loading ? (
              ...
              filtered.map((l) => {
                ...
                const name = l.projectName || l.contactName || l.company || 'Untitled';
                return (
                  <button
                    key={l.id}
                    ...
                    <div className="min-w-0 flex-1">
                      <p className={`text-sm truncate ${isCurrent ? 'font-semibold' : 'font-medium'}`}>{name}</p>
                      {l.company && l.projectName && (
                        <p className="text-[10px] text-muted-foreground/70 truncate">{l.company}</p>
                      )}
                    </div>
                    <span className="text-[10px] text-muted-foreground/50 shrink-0">{l.stage}</span>
