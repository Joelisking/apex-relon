...
export class ClientsService {
  ...
  async findAll(userId?: string, userRole?: string) {
    ...
    // CEO, ADMIN, DESIGNER, QS see all clients (no filter)
  ...
  async create(data: Record<string, unknown>, userId?: string) {
    ...
    this.workflowsService.triggerRules('CLIENT_CREATED', 'CLIENT', client.id, client as unknown as Record<string, unknown>);
  ...
  async update(id: string, data: Record<string, unknown>, userId?: string) {
    ...
    this.workflowsService.triggerRules('CLIENT_UPDATED', 'CLIENT', client.id, client as unknown as Record<string, unknown>);
  ...
  async bulkUpdate(ids: string[], data: Record<string, unknown>, userId?: string) {
    ...
    // Validate accountManagerId if provided — prevent silent FK violations.
    if (data.accountManagerId) {
      ...
      if (!manager) {
        throw new BadRequestException('Invalid account manager ID');
      }
    }
    ...
    if (accountManager !== undefined) {
      // accountManager relation cannot be used in updateMany — skip relational fields
    }
  ...
  async convertLeadToClient(
    ...
    const project = await this.prisma.project.create({
      data: {
        ...
        projectManagerId:
          projectManagerId || accountManagerId || lead.assignedToId,
        designerId:
          projectData?.designerId || lead.designerId || undefined,
   ...
   */
  private async enrichClientWithMetrics(client: { id: string; createdAt: Date; lifetimeRevenue: number; statusOverride?: boolean | null; status?: string | null; statusOverrideReason?: string | null; [key: string]: unknown }) {
    // Calculate metrics
