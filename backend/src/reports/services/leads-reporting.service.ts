...
export class LeadsReportingService {
  ...
  async getOverview(
    ...
    const conversionRate =
      closedCount > 0
        ? Math.round((wonLeads / closedCount) * 100)
        : 0;
    ...
    const stageProbabilityMap = new Map(
      pipelineStages.map((s) => [s.name, s.probability]),
    );
    const weightedForecast = Math.round(
      activeLeads.reduce((sum, l) => {
        const probability = stageProbabilityMap.get(l.stage) ?? 0;
        return sum + (l.expectedValue * probability) / 100;
      }, 0),
    );
    ...
    allLeads.forEach((lead) => {
      ...
      stageMap.set(lead.stage, {
        ...
        value: existing.value + lead.expectedValue,
      });
    });
  ...
  async getStageAnalysis(
    ...
    leadIds.forEach((leadId) => {
      ...
      leadStageHistory.forEach((entry, idx) => {
        ...
        existing.value += lead.expectedValue;
  ...
  async getRevenueByRep(
    ...
    return Array.from(repMap.entries())
      .map(([repId, data]) => {
        ...
        const avgDealSize =
          wonLeads > 0 ? Math.round(totalRevenue / wonLeads) : 0;
  ...
  private buildWhereClause(
    ...
    const where: Record<string, unknown> = this.buildRoleFilter(
      filters,
      user,
    );
    const dateRange = this.getDateRange(filters);
