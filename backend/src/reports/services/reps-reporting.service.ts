...
export class RepsReportingService {
  ...
  async getOverview(
    ...
    const totalContractedValue = performance.reduce(
      (sum, r) => sum + r.totalContractedValue,
      0,
    );

    const repsWithCycle = performance.filter(
      (r) => r.avgSalesCycle > 0,
    );
    const avgSalesCycle =
      repsWithCycle.length > 0
        ? Math.round(
            repsWithCycle.reduce(
              (sum, r) => sum + r.avgSalesCycle,
              0,
  ...
  async getPerformanceData(
    ...
    const reps = await this.prisma.user.findMany({
      where: await this.buildWhereClause(filters, user),
      include: {
        assignedLeads: {
          where: {
            stage: { in: ['Won', 'Lost'] },
        ...
        activities: {
          where: filters.period
            ? {
                createdAt: {
                  gte: this.getPeriodStart(filters.period),
                },
              }
            : undefined,
        },
    ...
    for (const rep of reps) {
      ...
      const totalContractedValue = wonLeads.reduce(
        (sum, l) => sum + (l.contractedValue ?? l.expectedValue),
        0,
      );
      ...
      const avgSalesCycle =
        leads.length > 0
          ? Math.round(
              leads.reduce((sum, l) => {
                const days =
                  (new Date(l.dealClosedAt!).getTime() -
    ...
    return performance.sort(
      (a, b) => b.totalContractedValue - a.totalContractedValue,
    );
  }

  async getStageTimeByRep(
    filters: ReportFiltersDto,
    user: ReportUser,
  ): Promise<RepStageTime[]> {
    const dateRange = this.getDateRange(filters);
    ...
    for (const rep of reps) {
      ...
      leadIds.forEach((leadId) => {
        const leadStageHistory = stageHistory.filter(
          (h) => h.leadId === leadId,
        );
        ...
        leadStageHistory.forEach((entry, idx) => {
          ...
          const existing = stageStats.get(entry.toStage) || {
            totalDays: 0,
            count: 0,
          };
      ...
      const stageData = Array.from(stageStats.entries()).map(
        ([stage, stats]) => ({
          stage,
  ...
  private async buildWhereClause(
    ...
    // Dynamically find roles that have 'leads:create' (sales-active roles).
    // This avoids hardcoding role names like ['SALES', 'BDM', 'CEO', 'ADMIN'].
    const rolesWithLeadsCreate =
      await this.prisma.rolePermission.findMany({
        where: { permission: 'leads:create' },
        select: { role: true },
      });
    const salesRoles = rolesWithLeadsCreate.map((r) => r.role);
    ...
    // Scope: canViewAll = all reps; has teamId = own team; else = self only
    if (!user.canViewAll) {
      if (user.teamId) {
