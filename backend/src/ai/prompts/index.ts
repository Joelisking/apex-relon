...
export function buildClientHealthPrompt(client: Record<string, unknown>): string {
  ...
  const recentActivitiesText = activities
    ...
    .map((a: Record<string, unknown>, idx: number) => {
      ...
      return `${idx + 1}. [${a.type}${daysAgo !== '?' ? ` - ${daysAgo} days ago` : ''}] ${detail}`;
    })
    .join('\n');
...
- Risk Level: ${(metrics.revenueConcentration as Record<string, unknown>)?.isHighRisk ? 'HIGH (>50% from top 5)' : 'HEALTHY'}
  ...
  projects
    ...
    .map(
      (p: Record<string, unknown>) =>
        `- ${p.name}: ${p.status}, $${(p.value as number | undefined)?.toLocaleString()}`,
    )
    .join('\n') || '- No projects yet'
}
...
Return JSON: { "summary": "...", "bottlenecks": [...], "winProbabilityByStage": {...}, "recommendations": [...], "urgentLeads": [...] }`;
}
